...
CInterferogram::CInterferogram(CParam *par, const int setn, const string time, const string name, const string baseline, const string m, const string s)
{
    PAR = par;
    SETN = setn;
    NAME = name;
    ...
    pM = new CImage(PAR, SETN, m, time);
    pS = new CImage(PAR, SETN, s, time);

    SPAN = pS->DATE - pM->DATE;
    BNDCOR = 1;
    OFFSET = 0;
    DATA = NULL; // do not allocate memory now because some interferograms may be deleted later
    MEAN = 0;
    STD = 0;
};
