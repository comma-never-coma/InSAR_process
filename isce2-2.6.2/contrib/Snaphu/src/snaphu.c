...
int UnwrapTile(infileT *infiles, outfileT *outfiles, paramT *params, 
  ...
  float **corr_mask, **pwr_mask, **mag_mask, cc_thres, pwr_thres;       /* mask para */
  long double pwr_sum=0.0L, count=0.0L;    /* mask para */
  long row_mask, col_mask;   /* mask para */
  incrcostT **incrcosts;
  void **costs;
  ...
  /************masking magnitude************/
  cc_thres=params->ccthreshold;
  pwr_thres=params->pwrthreshold;
  corr_mask = NULL;
  pwr_mask = NULL;
  
  mag_mask = (float **)malloc(nrow * sizeof(float *));
  if (mag_mask == NULL) {
    printf("Error: Memory allocation for mag_mask failed\n");
    return -1;
  }

  for (long i = 0; i < nrow; i++) {
    mag_mask[i] = (float *)malloc(ncol * sizeof(float));
    if (mag_mask[i] == NULL) {
      printf("Error: Memory allocation for mag_mask[%ld] failed\n", i);
      return -1;
    }
  }
  // Assign values of the mag array to the mag_mask array
  for (long i = 0; i < nrow; i++) {
    for (long j = 0; j < ncol; j++) {
      mag_mask[i][j] = mag[i][j];
    }
  }
  
  Read2DArray((void ***)&corr_mask,infiles->avecorfile,linelen,nlines,tileparams,
		sizeof(float *),sizeof(float));
  Read2DArray((void ***)&pwr_mask,infiles->avemlifile,linelen,nlines,tileparams,
		  sizeof(float *),sizeof(float));
  
  for(row_mask = 0; row_mask < nrow; row_mask++){
    for(col_mask = 0; col_mask < ncol; col_mask++){
      pwr_sum += pwr_mask[row_mask][col_mask];
      count++;
      }
  }
  if (count == 0.0L) {
    printf("Error: No data in pwr_mask for calculation\n");
    return -1;
  }
  float pwr_mean = (float)(pwr_sum / count);
  
  float pwr_threshold = pwr_thres * pwr_mean;
  
  fprintf(sp1,"Masking pixels below the corherence threshold %f\n", cc_thres);
  for(row_mask=0;row_mask<nrow;row_mask++){
    for(col_mask=0;col_mask<ncol;col_mask++){
      if(corr_mask[row_mask][col_mask]<cc_thres){
	mag_mask[row_mask][col_mask]=0.0f;
      }
    }
  }
  fprintf(sp1,"Masking pixels below the intensity threshold %f\n", pwr_threshold);
  for(row_mask=0;row_mask<nrow;row_mask++){
    for(col_mask=0;col_mask<ncol;col_mask++){
      if(pwr_mask[row_mask][col_mask]<pwr_threshold){
	mag_mask[row_mask][col_mask]=0.0f;
      }
    }
  }
  Free2DArray((void **)corr_mask, nrow);
  Free2DArray((void **)pwr_mask, nrow);
  /************masking magnitude************/
  
  /* read mask file and apply to magnitude */
  ReadByteMask(mag_mask,infiles,linelen,nlines,tileparams,params);

  /* make sure we have at least one pixel that is not masked */
  allmasked=CheckMagMasking(mag_mask,nrow,ncol);  
  ...
  BuildCostArrays(&costs,&mstcosts,mag_mask,wrappedphase,unwrappedest,
  ...
  if(params->eval){
    ...
    Free2DArray((void **)mag_mask,nrow);
  ...
  if(!params->unwrapped){
    ...
    if(params->initonly || strlen(outfiles->initfile)){
      ...
      if(params->initonly){
        ...
        WriteOutputFile(mag_mask,unwrappedphase,outfiles->outfile,outfiles,
                        nrow,ncol);  
        Free2DArray((void **)mag_mask,nrow);
        Free2DArray((void **)wrappedphase,nrow);
        ...
        WriteOutputFile(mag_mask,unwrappedphase,outfiles->initfile,outfiles,
                        nrow,ncol);  
        Free2DArray((void **)unwrappedphase,nrow);
      }
    }
  ...
  MaskNodes(nrow,ncol,nodes,ground,mag_mask);
  ...
  if(!allmasked){
    ...
    while(TRUE){ 
      ...
      sourcelist=NULL;
      nconnectedarr=NULL;
      nsource=SelectSources(nodes,mag_mask,ground,nflow,flows,ngroundarcs,
      ...
      for(isource=0;isource<nsource;isource++){
        ...
        n+=TreeSolve(nodes,NULL,ground,source,
                     ...
                     ngroundarcs,nflow,mag_mask,wrappedphase,outfiles->outfile,
      ...
      mostflow=MaxNonMaskFlow(flows,mag_mask,nrow,ncol);
