...
PyObject *setCoherenceThreshold_C(PyObject *self, PyObject *args)
{
    double ccthres;
    if(!PyArg_ParseTuple(args,"d", &ccthres))
    {
        return NULL;
    }
    params->ccthreshold = ccthres;

    return Py_BuildValue("i",0);
}

PyObject *setIntensityThreshold_C(PyObject *self, PyObject *args)
{
    double pwrthres;
    if(!PyArg_ParseTuple(args, "d", &pwrthres))
    {
        return NULL;
    }
    params->pwrthreshold = pwrthres;

    return Py_BuildValue("i",0);
}

PyObject *setAverageCoherence_C(PyObject *self, PyObject *args)
{
    char *avecor;
    if(!PyArg_ParseTuple(args,"s", &avecor))
    {
        return NULL;
    }
    StrNCopy(infile->avecorfile, avecor, MAXSTRLEN);

    return Py_BuildValue("i",0);
}

PyObject *setAverageIntensity_C(PyObject *self, PyObject *args)
{
    char *avemli;
    if(!PyArg_ParseTuple(args,"s", &avemli))
    {
        return NULL;
    }
    StrNCopy(infile->avemlifile, avemli, MAXSTRLEN);
