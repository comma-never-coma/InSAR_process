...
##########################################################
# Script that:
# -ploting pslike deformation maps
#       
#########################################################################################################
...
if [ $# -ne 7 ]; then
        ...
        echo "      geo_tcp: pslike points 'geo_tcp' generated by select_pslike.py "
        exit 1
fi
...
gmt grdtrack $geo_tcp -Ggeo_velocity.grd > geo_velocity.xyz
...
cat ts.tab | while read a;do
	...
	gmt grdtrack $geo_tcp -G$a.grd > $a.xyz
