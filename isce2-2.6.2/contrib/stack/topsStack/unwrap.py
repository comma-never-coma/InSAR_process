...
def createParser():
    ...
    parser.add_argument('--cc_thres', dest='coherenceThreshold', type=float, default=0.4,
            help='coherence threshold for masking')
    parser.add_argument('--pwr_thres', dest='intensityThreshold', type=float, default=0.1,
            help='relative intensity threshold for masking')
...
def runUnwrap(infile, outfile, corfile, config, ccThres, pwrThres, avecor, avemli, costMode = None,initMethod = None, defomax = None, initOnly = None):
    ...
    snp.setCoherenceThreshold(ccThres)
    snp.setIntensityThreshold(pwrThres)
    snp.setAverageCoherence(avecor)
    snp.setAverageIntensity(avemli)
...
def runUnwrapMcf(infile, outfile, corfile, config, ccThres, pwrThres, avecor, avemli, defomax=2):
    runUnwrap(infile, outfile, corfile, config, ccThres, pwrThres, avecor, avemli, costMode = 'SMOOTH',initMethod = 'MCF', defomax = defomax, initOnly = True)
    return
...
def main(iargs=None):
    ...
    if inps.method == 'snaphu':
        ...
        ave_cor = os.path.join(os.path.dirname(inps.reference), 'merged/interferograms/ave.cor')
        ave_mli = os.path.join(os.path.dirname(inps.reference), 'merged/SLC/ave.mli')
        fncall(inps.intfile, inps.unwfile, inps.cohfile, metadata, inps.coherenceThreshold, inps.intensityThreshold, ave_cor, ave_mli, defomax=inps.defomax)
