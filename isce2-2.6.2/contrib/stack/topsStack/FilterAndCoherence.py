...
def runFilter(infile, outfile, filterStrength, FFTwindow):
    ...
    filtImage1 = isceobj.createIntImage()
    filtImage1.setFilename(outfile + '_1')
    filtImage1.setWidth(intImage.getWidth())
    filtImage1.setAccessMode('write')
    filtImage1.createImage()
    ...
    objFilter1 = Filter()
    objFilter1.wireInputPort(name='interferogram',object=intImage)
    objFilter1.wireOutputPort(name='filtered interferogram',object=filtImage1)
    objFilter1.goldsteinWerner(alpha=filterStrength, nfft=FFTwindow)

    objFilter = Filter()
    objFilter.wireInputPort(name='interferogram',object=filtImage1)
    objFilter.wireOutputPort(name='filtered interferogram',object=filtImage)
    objFilter.goldsteinWerner(alpha=0.5, nfft=32)

    intImage.finalizeImage()
    filtImage1.finalizeImage()
    filtImage.finalizeImage()
    os.remove(outfile + '_1')
    os.remove(outfile + '_1.vrt')
    os.remove(outfile + '_1.xml')
...
def createParser():
    ...
    parser.add_argument('-s', '--strength', type=float, default=0.5, help='Filter strength',
            dest='filterstrength')
    parser.add_argument('--nfft', type=int, default=32, help='Filter FFT window size',
            dest='fftwindow')
...
def main(iargs=None):
    ...
    runFilter(inps.infile, inps.filtfile, inps.filterstrength, inps.fftwindow)
