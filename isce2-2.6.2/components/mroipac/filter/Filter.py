...
class Filter(Component):
    ...
    def goldsteinWerner(self, alpha=0.5, nfft=32):
        """
        ...
        self.logger.debug("filter FFT window size: %s"%(nfft))

        # Filter the interferometric phase
        self.logger.info("Filtering interferogram")
        self._psfilt(input,output,width,length,alpha,nfft)
        self._rescale_magnitude(input,output,width,length)
        self.filteredImage.renderHdr()

    def _psfilt(self,input,output,width,length,alpha,nfft):
        """
        ...
        @param nfft the FFT window size of filtering
        """
        ...
        nfft_c = ctypes.c_int(nfft) # FFT window size
        step_c = ctypes.c_int(nfft//2) # Stepsize in range and azimuth for the filter(default=nfft/8) (must be nfft/2)?
        ...
        lib.psfilt(input_c,output_c,width_c,length_c,alpha_c,nfft_c,step_c,xmin_c,xmax_c,ymin_c,ymax_c)
