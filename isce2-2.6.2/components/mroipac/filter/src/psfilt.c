...
void psfilt(char *int_filename, char *sm_filename, int width, int nlines, double alpha, int fft, int step, int xmin, int xmax, int ymin, int ymax)
{
   ...
   fprintf(stdout,"filter nfft, step: %5d %5d\n",fft,step);
   ...
   cmp = cmatrix(0, fft-1, -fft,width+fft);			/* add space around the arrays */ 
   sm = cmatrix(0,fft-1,-fft,width+fft); 
   ...
   for (i=0; i < fft; i++){
     for (j=0; j < fft; j++){
       azw = 1.0 - fabs(2.0*(double)(i-fft/2)/(fft+1));  
       rw  = 1.0 - fabs(2.0*(double)(j-fft/2)/(fft+1));  
       wf[i][j]=azw*rw/(double)(fft*fft);
#ifdef DEBUG
   ...
   for (i=0; i < yh; i += step){
     for(i1=step; i1 < fft; i1++){
       fread((char *)cmp[i1], sizeof(fcomplex), width, int_file);
       if (feof(int_file) != 0){				/* fill with zero if at end of file */
         for(j1= -fft; j1 < width+fft; j1++){cmp[i1][j1].re=0.0; cmp[i1][j1].im=0.0;}
       }
       for(j1= -fft; j1 < width+fft; j1++){
   ...
   /*free_cmatrix(cmp,0, fft-1, -fft,width+fft);
   free_cmatrix(sm,0,fft-1,-fft,width+fft);*/
   fclose(int_file);
   fclose(sm_file);
} 
 
void psd_wgt(fcomplex **cmp, fcomplex **seg_fft, double alpha, int fft, int ix, int iy)
/* 
...

{
  ...
  ndim=2, isign = -1, nfft[1]=fft, nfft[2]=fft, nfft[0]=0; /* fft initialization */
  
  for (i=0; i < fft; i++){  					/* load up data array */
    for (j=ix; j < ix+fft; j++){
