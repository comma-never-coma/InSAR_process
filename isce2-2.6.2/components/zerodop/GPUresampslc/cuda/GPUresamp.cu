...
__global__ void GPUResamp(struct InputData inData) {
    ...
    // check within outWidth*outLength
    if (pix >= (ini[2] * ini[6]))
        return;
    ...
    int ifraca = min(max(0,int(fraca*SINC_SUB)),SINC_SUB-1); 
    int ifracr = min(max(0,int(fracr*SINC_SUB)),SINC_SUB-1);
    ...
    for (int i=-SINC_HALF+1; i<=SINC_HALF; i++) {
        ...
        for (int j=-SINC_HALF+1; j<=SINC_HALF; j++) {
            double weight = inData.fintp[IDX1D(ifraca,SINC_HALF-i,SINC_LEN)]
                    ...
                    double *dopPoly, double *azCarrierPoly, double *rgCarrierPoly, float *fintp)
{
     ...
     *  Int 1 - inWidth
     *  Int 2 - outWidth
     *  Int 3 - firstImageRow //0
     *  Int 4 - firstTileRow //0
     *  Int 5 - nRowsInBlock //outLength
     *  Int 6 - outLength
     *  Int 7 - flatten
     *
     * * * * * * * * * * * * * * * * * * * */
   ...
   // remove carriers from the input image
    ...
    /*int threads = 128;
    dim3 block(threads);
    dim3 grid((nInPix + (threads - 1)) / threads);
    //if ((grid.x * threads) > nInPix) printf("    (NOTE: There will be %d 'empty' threads).\n", ((grid.x*threads)-nInPix));
    removeCarrier <<<grid, block>>>(inData);
    checkKernelErrors();
    dim3 block2(threads);
    dim3 grid2((nOutPix + (threads - 1)) / threads);
    if ((grid2.x * threads) > nOutPix) printf("    (NOTE: There will be %d 'empty' threads).\n", ((grid2.x*threads)-nOutPix));
    GPUResamp <<<grid2, block2>>>(inData);
    checkKernelErrors();*/
