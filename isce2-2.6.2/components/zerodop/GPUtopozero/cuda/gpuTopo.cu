...
__global__ void runTopo(struct Orbit orbit, struct OutputImgArrs outImgArrs, struct InputImgArrs inImgArrs, long NPIXELS, long OFFSET) {
    ...
    if (pixel < NPIXELS) { // Make sure we're not operating on a non-existent pixel
        ...
        // add
        double n_img[3], n_img_enu[3], n_trg_enu[3], cospsi;
        // add
        ...
        * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
        ...
        // n_img(3), n_img_enu(3), n_trg_enu(3), cospsi
        cross(delta, velsat, n_img); 
        n_img_enu[0] = (enumat[0][0] * -d_inpts_dbl[12] * n_img[0]) + (enumat[0][1] * -d_inpts_dbl[12] * n_img[1]) + (enumat[0][2] * -d_inpts_dbl[12] * n_img[2]);
        n_img_enu[1] = (enumat[1][0] * -d_inpts_dbl[12] * n_img[0]) + (enumat[1][1] * -d_inpts_dbl[12] * n_img[1]) + (enumat[1][2] * -d_inpts_dbl[12] * n_img[2]);
        n_img_enu[2] = (enumat[2][0] * -d_inpts_dbl[12] * n_img[0]) + (enumat[2][1] * -d_inpts_dbl[12] * n_img[1]) + (enumat[2][2] * -d_inpts_dbl[12] * n_img[2]);
        n_trg_enu[0] = -alpha;
        n_trg_enu[1] = -beta;
        n_trg_enu[2] = 1.0;
        cospsi = dot(n_trg_enu, n_img_enu) / (norm(3,n_trg_enu) * norm(3,n_img_enu));
        thrd_incang0 = acos(cospsi) * (180. / M_PI);
